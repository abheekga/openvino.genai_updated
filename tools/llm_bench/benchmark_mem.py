...
def main():
    ...
    def track_memory(interval=0.001):
        nonlocal peak_system_mem, running, peak_gpu_mem
        while running:
            gpu_mem_used = get_rss_memory()
            mem_used = psutil.virtual_memory().used
            if mem_used > peak_system_mem:
                peak_system_mem = mem_used
            if gpu_mem_used > peak_gpu_mem:
                peak_gpu_mem = gpu_mem_used
            time.sleep(interval)
    ...
    try:
        ...
        print(f"Peak system memory during inference was: {peak_system_mem / (1024**3):.2f} GB")
        print(f"Peak GPU memory was: {peak_gpu_mem / (1024**3):.2f} GB")
    except Exception:
